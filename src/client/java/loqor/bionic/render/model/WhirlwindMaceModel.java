...
public class WhirlwindMaceModel extends EntityModel<Entity> {
    ...
    public WhirlwindMaceModel(ModelPart root) {
        ...
        this.wind_handle = this.handle.getChild("wind_handle");
        ...
        this.mace = this.bone.getChild("mace");
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData wind_handle = handle.addChild("wind_handle", ModelPartBuilder.create(), ModelTransform.of(-0.5F, -8.025F, 0.5F, 0.0F, 0.0F, -3.1416F));
        ...
        ModelPartData mace = bone.addChild("mace", ModelPartBuilder.create(), ModelTransform.pivot(0.0F, -20.025F, 0.0F));
    ...
    public void setItemModelAngles(PlayerEntity player, ItemStack stack) {
        ...
        if (player.getMainHandStack() == stack && player.isUsingItem()) {
            ...
            float useTimeRot = useTime > 40 ? useTime * 2.0f : Math.max(0, Math.min(40, 1.0f + useTime * 0.3f));
            this.wind_charge.yaw = useTimeRot;
            this.wind_outer.yaw = useTimeRot;
            this.wind_inner.yaw = -useTimeRot;
        } else {
            this.wind_charge.yaw = rot / 2f;
            this.wind_outer.yaw = rot / 2f;
            this.wind_inner.yaw = -rot / 2f;
        }
        this.wind_handle_top.yaw = rot;
        this.wind_handle_bottom.yaw = rot;
        this.wind_handle_center.yaw = -rot;
    }
}